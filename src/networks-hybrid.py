...
class InpaintGenerator(BaseNetwork):
    def __init__(self, residual_blocks=8, init_weights=True):
        ...
        for _ in range(residual_blocks):
            block = ResnetBlock(256, 4)     #add dilation
            blocks.append(block)
        ...
        self.fusion64 = FeatureFusion(64)
        self.fusion128 = FeatureFusion(128)
        self.fusion256 = FeatureFusion(256)
        ...
        # self.atten256_2 = localTransformer(in_channels=256, depth=4)
    ...
    def forward(self, x):      
        ...
        x1 = self.fusion64(x1,x1_1)

        x2 = self.encoder2(x1)           #size 256  channels 128
        x2_1 = self.vitblock256(x2)
        x2 = self.fusion128(x2,x2_1)

        x3 = self.encoder3(x2)      #size 128 channels 256        
        x3_1 = self.vitblock128(x3)
        x3 = self.fusion256(x3,x3_1)

        x1_1a = self.encoder1_1(x)
        x1_2a = self.encoder1_2(x1_1a)
        x1_3a = self.encoder1_3(x1_2a)
        x1_4a = self.encoder1_4(x1_3a)
        
        x3_local = self.atten256(x1_4a)
        x3 = self.encoder4(torch.cat((x3,x3_local),dim=1))
