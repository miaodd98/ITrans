...
class InpaintGenerator(BaseNetwork):
    def __init__(self, residual_blocks=8, init_weights=True):
        ...
        self.vitblock1 = ViT(image_size=512, patch_size=16, dim=64, depth=2, heads=8)
        self.vitblock2 = ViT(image_size=256, patch_size=16, dim=128, depth=2, heads=8)
        self.vitblock3 = ViT(image_size=128, patch_size=16, dim=256, depth=2, heads=8)

        # self.atten64 = localTransformer(in_channels=64, depth=4)
        # self.atten128 = localTransformer(in_channels=128, depth=4)
        self.atten256 = localTransformer(in_channels=256, depth=8)

        self.decoder3 = nn.Sequential(
            nn.ConvTranspose2d(in_channels=256+256+256, out_channels=128, kernel_size=4, stride=2, padding=1),
            nn.InstanceNorm2d(128, track_running_stats=False),
            nn.ReLU(True)
        )
        ...
        self.decoder1 = nn.Sequential(
            nn.ReflectionPad2d(3),
            nn.Conv2d(in_channels=64+64, out_channels=3, kernel_size=7, padding=0)
        )
    ...
    def forward(self, x):
        ...
        x1_1 = self.encoder1_1(x)
        x1_2 = self.encoder1_2(x1_1)
        x1_3 = self.encoder1_3(x1_2)
        x1_4 = self.encoder1_4(x1_3)

        x4 = self.middle(x3)
        x_local = self.atten256(x1_4)

        x33 = self.vitblock3(x3)
        x22 = self.vitblock2(x2)
        x11 = self.vitblock1(x1)
